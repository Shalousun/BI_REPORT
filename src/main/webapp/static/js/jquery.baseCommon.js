...
;(function (factory) {
    ...
    $.boco = {
        /**
         * Convert IC card number to 10 hex
         * 将ic存储芯片中的8位16进制编码转化成10进制
         * usage:$.boco.strHex2Long("0184F3B9");//3119744001
         * @param str
         * @returns {string}
         */
        strHex2Long: function (str) {
            var buffer = [];
            buffer.push("00000000");//
            var length = (str.length >> 1) - 1;
            for (var i = length; i >= 0; i--) {
                buffer.push(str.substring(i << 1, (i + 1) << 1));
            }
            var value = parseInt(buffer.join(''), 16);
            var value1 = ~value + 1;
            value1 = value1 >>> 0;
            return value.toString() + "（" + value1.toString() + "）";
        },
        /**
         * Convert 10 hex number to 16 hex
         * 将10进制编码转化成8位16进制编号
         * usage:$.boco.long2HexStr(3119744001);//0184F3B9
         * @param number
         * @returns {string}
         */
        long2HexStr: function (number) {
            var strHex = parseInt(number, 10).toString(16);//to hex
            strHex = (strHex.length == 8) ? strHex : '0' + strHex;
            var buffer = [];
            var length = (strHex.length >> 1) - 1;
            for (var i = length; i >= 0; i--) {
                buffer.push(strHex.substring(i << 1, (i + 1) << 1));
            }
         ...
         */
        getFistDayOfLastMonth: function (ms) {
